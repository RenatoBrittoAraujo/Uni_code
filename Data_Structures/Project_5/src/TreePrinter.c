...
printNode * buildAsciiTreeRecursive(Tree * t) 
{
	printNode * node;
...
printNode * buildAsciiTree(Tree * t) 
{
	printNode * node;
	if (t == NULL)
		return NULL;
...
void destroyAsciiTree(printNode *node) 
{
	if (node == NULL)
		return;
...
void leftProfile(printNode * node, int x, int y) 
{
	if (node == NULL)
		return;
	int i, isleft;
...
void rightProfile(printNode *node, int x, int y) 
{
	int i, notleft;
	if (node == NULL)
		return;
...
void computeEdgeLenght(printNode *node) 
{
	int h, hmin, i, delta;
	if (node == NULL) return;
...
void printLevel(printNode *node, int x, int level) 
{
	if (node == NULL)
		return;
	int i, isleft;
	...
	{
		printLevel(node->left, x-node->edge_length-1, level-node->edge_length-1);
		printLevel(node->right, x+node->edge_length+1, level-node->edge_length-1);
	}
}
